...
var BABYLON;
(function (BABYLON) {
    BABYLON.Effect.ShadersStore["pixelateFragmentShader"] = "precision highp float;\n\nvarying vec2 vUV;\n\nuniform vec4 dimensions;\nuniform vec2 pixelSize;\nuniform sampler2D uSampler;\n\nvoid main(void)\n{\n    vec2 coord = vUV;\n\n    vec2 size = dimensions.xy / pixelSize;\n\n    vec2 color = floor((vUV * size)) / size + pixelSize / dimensions.xy * 0.5;\n\n    gl_FragColor = texture2D(uSampler, color);\n}";
    var PixelatePostProcess = (function (_super) {
        __extends(PixelatePostProcess, _super);
        function PixelatePostProcess(name, options, _postProcessTexture, camera, samplingMode, engine, reusable) {
            var _this = this;
            if (reusable === void 0) { reusable = true; }
            _super.call(this, name, "pixelate", ["pixelSize", "dimensions"], ["uSampler"], 1, camera, samplingMode, engine, reusable);
            this.dimensions = new BABYLON.Vector4(800, 400, 0, 0);
            this.pixelSize = new BABYLON.Vector2(15.0, 15.0);
            this.postProcessTexture = _postProcessTexture;
            this.onApplyObservable.add(function (effect) {
                effect.setTextureFromPostProcess("uSampler", _this.postProcessTexture);
                effect.setVector2("pixelSize", _this.pixelSize);
                effect.setVector4("dimensions", _this.dimensions);
            });
        }
        return PixelatePostProcess;
    }(BABYLON.PostProcess));
    BABYLON.PixelatePostProcess = PixelatePostProcess;
})(BABYLON || (BABYLON = {}));
